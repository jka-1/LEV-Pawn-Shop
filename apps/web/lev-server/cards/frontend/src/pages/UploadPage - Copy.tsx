...
export default function UploadPage() {
  ...
  async function estimateWithAI() {
    ...
    setAiLoading(true);
    try {
      // Optional: try to grab approximate location (lat/lng only)
  ...
  const onSubmit: React.FormEventHandler = async (e) => {
    ...
    // 🌟 Require AI to have filled in a price
    if (!draft.price.trim()) {
      return setError("Please use AI to estimate a price before adding the item.");
    }

    const priceNum = Number(draft.price);
    if (!Number.isFinite(priceNum) || priceNum < 0) {
      return setError(
        "AI returned an invalid price. Please try estimating again."
      );
    }
  ...
  return (
    <div className="page page--auth">
      {/* removed 'upload-page' to avoid internal scrollbars */}
      <NavBar />
        ...
        >
          ...
          <div className="auth-card">
            <form onSubmit={onSubmit} className="upload-form">
              ...
              {/* 🌟 AI price (read-only) */}
              <div>
                <label htmlFor="up-price">AI Price (USD)</label>
                <input
                  id="up-price"
                  className="input"
                  inputMode="decimal"
                  placeholder="Use AI to estimate"
                  value={draft.price}
                  // No onChange — we want AI to control this
                  readOnly
                />
                <div
                  style={{
